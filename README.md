...
### Проблема с PostgreSQL при 300 млн карточек товара
...
Отлично — ниже полный практический план и архитектурная схема для хранения \~300 млн карточек товара в ScyllaDB, с реальными цифрами, примером схемы (CQL), рекомендациями по репликации/консистенции, миграции из PostgreSQL и расчётом ёмкости/узлов. Я сделал расчёты и дал варианты (консервативные и экономные), чтобы ты мог выбрать по бюджету и целям.

# Краткая идея

ScyllaDB — распределённая, горизонтально масштабируемая колонко-ориентированная NoSQL БД, совместимая с Cassandra CQL, но реализованная на C++/Seastar для гораздо более эффективного использования CPU/памяти/SSD и низкой предсказуемой латентности. Это делает её отличным выбором для больших каталогов товаров с высокой QPS и большими объёмами данных. ([scylladb.com][1], [enterprise.docs.scylladb.com][2])

---

# 1) Оценка объёма хранения (быстрое число)
...
* Если тебе нужны сложные аналитические запросы — держи OLAP-реплику в ClickHouse / Druid / Elastic / Redshift и т.д.
...
* Планируй мониторинг больших партиций и tombstones; не давай партициям вырасти слишком большими.
...
* Избегай больших партиций: ограничивай размер партиции (в байтах или числом строк), используйте bucket-ы. ([scylladb.com][1])
* Тщательно подумай о tombstones / mass deletes: массовые удаления создают проблемы. Планируй TTL и удаление аккуратно. ([forum.scylladb.com][10])
...
# 11) Краткое сравнение PostgreSQL → Scylla для твоего сценария
...
2. Поднять тестовый кластер Scylla (3–6 узлов) с NVMe/16–32 vCPU/128GB RAM. ([scylladb.com][3])
3. Разработать CQL-схему (product\_by\_id + bucketed products\_by\_category).
4. Прототип миграции: экспорт 1–10% данных -> загрузить через Spark + scylla-spark-connector или Scylla Migrator и валидировать. ([enterprise.docs.scylladb.com][12], [GitHub][11])
...
## Курсы, примеры, учебные материалы

### 3. ScyllaDB University

Бесплатные структурированные онлайн-курсы:

* S101: ScyllaDB Essentials — основы NoSQL и ScyllaDB.
* S201: Data Modeling and Application Development.
* S301: ScyllaDB Operations, S210: Using ScyllaDB Drivers и другие.
  Содержат видео-уроки, практические лаборатории, тесты и сертификаты. ([ScyllaDB University][3])

### 4. Tutorials and Example Projects

Практика в действии:

* Media Player, Gaming Leaderboard, Video Streaming, Shopping Cart, Feature Store и другие проекты, реализованные с использованием ScyllaDB (Node.js, Java, Python, Rust и др.). ([docs.scylladb.com][4])

### 5. Learn to Use ScyllaDB

Собрано всё обучающее:

* Курсы (University), example projects, вебинары, Tech Talks, ScyllaDB Summit, форум, блог и др. ([docs.scylladb.com][5])

---

## Видео и медиа-ресурсы

### 6. YouTube: What is ScyllaDB? A Quick Start Guide for Beginners

Краткое вводное 6-минутное видео, объясняющее суть ScyllaDB. ([YouTube][6])

### 7. YouTube: Getting Started with NoSQL, Using ScyllaDB

Видео о NoSQL и ScyllaDB, подходит для первой ориентации. ([YouTube][7])

### 8. ScyllaDB YouTube / Video Library

Официальный видеоканал с роликами:

* «ScyllaDB vs Cassandra: 5 Minute Demo»
* «Predictable Performance at Scale to 2M OPS»
* «Inside Tripadvisor’s Real-time Personalization with ScyllaDB»
* «Sizing Considerations for ScyllaDB» и другие. ([resources.scylladb.com][8])

---

## Справочные и обзорные ресурсы

### 9. Статья в Wikipedia

Место с базовыми фактами:

* Год релиза (2015), язык (C++), библиотека Seastar, совместимость с Cassandra и DynamoDB API, производительность и архитектурные особенности. ([Википедия][9])
...
Если хочешь, могу помочь с первым шагом — порекомендую подходящий курс (Essentials или Operations), или предложу небольшой видеоролик по моделированию данных & паттернам — просто скажи, какая область тебе интереснее сейчас!
